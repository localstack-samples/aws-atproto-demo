...
- Each user has a personal data repository identified by a **DID** (decentralized identifier).
...
- The **firehose** is the real-time stream of commits observed by a relay service across the network. Bluesky's [Jetstream](https://github.com/bluesky-social/jetstream) service exposes one such feed as plain JSON over WebSocket (this is easier to consume than the raw binary firehose, which uses CBOR/CAR encoding).
...
| Fan-out mechanism  | **EventBridge**, not direct invocation or SNS                                                                                               | EventBridge is AWS's event bus. Content-based filtering lets each consumer subscribe to exactly what it needs without the ingest side knowing any of them exist. Consumers can be added or removed independently. Failed target delivery goes to a shared SQS queue; failures after Lambda accepts an event go to a per-function SQS queue. |
| Raw storage        | **S3 raw archive**                                                                                                                         | The poller writes each drained batch to S3 before publishing it. The Fargate consumer batches writes in memory and publishes to EventBridge first, so its archive can lag or miss an event after a crash or S3 write failure. Treat S3 as a best-effort audit and reprocessing input in this demo, not the source of truth. |
...
Both ingestion stacks normalize events onto EventBridge and maintain an S3 raw archive. Their durability timing differs: the poller archives each batch before fan-out, while the Fargate consumer buffers archive writes and publishes first. Neither one has any idea the analytics, moderation, or notifications Lambdas exist, which is the whole point of the pattern.
...
lambdas/
  analytics/           counts events per type, using the source event hour
...
This stands up S3, three DynamoDB tables (including the ingest cursor), the notification SQS queue and failure queues, the
...
The mock firehose sends ~35 events on a slow loop. The Quickstart exercises the full pipeline (ingest → S3 raw archive + EventBridge → three fan-out Lambdas) at a volume your laptop should comfortably emulate. LocalStack handles that workload well, and repeated `cdk deploy` / invoke cycles are exactly the type of local testing LocalStack is designed for.
...
- **`ingest-consumer/`** (used by `infra-fargate/`) holds the WebSocket connection open, buffers raw events, maps each event's ATProto `collection` field to an EventBridge detail-type (`pub.leaflet.document` → `document.created`, etc.), and publishes it immediately. It flushes accumulated NDJSON to S3 on a count/time threshold, so the archive is best-effort and can lag EventBridge. It saves the last successfully published Jetstream sequence cursor in DynamoDB and uses it to resume after a disconnect.
...
- **`lambdas/analytics`** matches every event on the bus and increments a per-event-type counter in DynamoDB, bucketed by the source event's hour.
- **`lambdas/moderation`** is only invoked for `comment.created` events whose `record.plaintext` matched a keyword list via EventBridge's wildcard content filtering. A comment can match more than one keyword rule, and EventBridge delivery is at-least-once, so the Lambda uses the stable Jetstream cursor as its DynamoDB key and records one flag per source event. (`pub.leaflet.document` has no plaintext field of its own — its content lives in nested block pages — so moderation only ever fires on comments, not documents. EventBridge filters are also case-sensitive and match substrings literally, not by meaning; a full-featured moderation system would apply semantic analysis or human review on top of keyword filters.)
...
- **Failure queues** preserve unsuccessful downstream deliveries for inspection and redrive: `atproto-target-delivery-failures` receives events EventBridge could not hand to a target, while each Lambda has its own `*-processing-failures` queue for events that exhausted Lambda's asynchronous retries after being accepted.
